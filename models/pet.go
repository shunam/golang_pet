...
func GetPetById(id int64) (v *Pet, err error) {
	o := orm.NewOrm()
	v = &Pet{Id: id}
	if err = o.QueryTable(new(Pet)).Filter("Id", id).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}
...
// UpdateImagePet updates Pet by Id (only photo) and returns error if
// the record to be updated doesn't exist
func UpdatePhotoPetById(m *Pet, fileName string) (err error) {
	o := orm.NewOrm()
	v := Pet{Id: m.Id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		m.Photo = fileName
		if num, err = o.Update(m, "Photo"); err == nil {
			fmt.Println("Number of records updated in database:", num)
		}
	}
	return
